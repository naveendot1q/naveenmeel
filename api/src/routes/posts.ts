...
import { db, postsTable } from "../db.js";
...
function getToken(req: any): string | undefined {
  return req.cookies?.admin_token || req.headers["x-admin-token"];
}
...
router.get("/posts", async (req, res) => {
  try {
    ...
    const isAdmin = !!(await verifyAdminToken(getToken(req)));
...
router.post("/posts", async (req, res) => {
  const admin = await verifyAdminToken(getToken(req));
